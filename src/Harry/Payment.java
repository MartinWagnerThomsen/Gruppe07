...
public class Payment {
    ...
    // Marker betaling som betalt
    public void markAsPaid() {
        this.isPaid = true;
        this.paymentDate = LocalDate.now();
        this.paymentTime = LocalTime.now();
        System.out.println("Payment of " + String.format("%.2f", amount) +
                " DKK marked as paid on " + paymentDate + " at " + paymentTime);
    }

    // Få betalingsstatus
    public String getPaymentStatus() {
        if (isPaid) {
            return "PAID on " + paymentDate + " at " + paymentTime + " via " + paymentMethod;
        } else {
            return "UNPAID - Amount due: " + String.format("%.2f", amount) + " DKK";
        }
    }
    ...
    public String getPaymentDetails() {
        return "Payment Details:\n" +
                "Amount: " + String.format("%.2f", amount) + " DKK\n" +
